...
@torch.no_grad()
def calcu_weight(pred: torch.Tensor, mask: torch.Tensor, alpha: float, beta: float, *kss):
    ...
    weights = alpha * (w_mask + diff * w_pred) + beta
    return weights

import torch.nn.functional as F
def structure_loss(pred, mask, alpha, beta, *ks):
    sig_pred = torch.sigmoid(pred)
    weit = calcu_weight(sig_pred, mask, alpha, beta, *ks)
