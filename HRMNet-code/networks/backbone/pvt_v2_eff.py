...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # x = attn @ v
        # Use scaled_dot_product_attention with autocast for better compatibility
...
class PyramidVisionTransformerV2(nn.Module):
    ...
    def forward(self, x):
        ...
        # Return tuple for compatibility with VGSformer (expects x0,x1,x2,x3 unpacking)
        return (
...
_BUILDERS = {
    ...
    "b4": pvt_v2_eff_b4,
    "b5": pvt_v2_eff_b5,
    "b2_li": pvt_v2_eff_b2_li,
}


def build_pvtv2(config, img_size, pt_root):
    """构建 PVT 骨干网络，支持加载预训练权重。

    Args:
        config: 配置对象，需包含 config.PVT.VARIANT (b0/b1/b2/b3/b4/b5/b2_li)。
                若 pt_root 非空，需 config.CKPT_NAME 指定 checkpoint 文件名。
        img_size: 输入图像尺寸。
        pt_root: 预训练权重根目录，为 None 时不加载。

    Returns:
        model: PyramidVisionTransformerV2 模型（已加载预训练权重若 pt_root 非空）
        embed_dims: 各 stage 的 embed_dim 列表 [C1, C2, C3, C4]
    """
    import os

    variant = config.BACKBONE.TYPE
    model = _BUILDERS[variant](pretrained=False, img_size=img_size)
    embed_dims = _PVT_EMBED_DIMS[variant]

    if pt_root:
        ckpt_path = os.path.join(pt_root, config.BACKBONE.CKPT_NAME)
        try:
