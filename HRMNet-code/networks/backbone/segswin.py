...
def build_segswin(config, img_size, pt_root):
    import os
    print(config)
    model = SegSwinTransformer(pretrain_img_size=img_size,
    ...
    if pt_root:
        ckpt_path = os.path.join(pt_root,config.CKPT_NAME)
        state_dict = torch.load(ckpt_path,map_location="cpu")
        safe_load_model(model,state_dict['model'],ckpt_path)
    embed_dims = [config.SWIN.EMBED_DIM*2**i for i in range(4)]
